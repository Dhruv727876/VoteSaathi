...
export const ElectionTimeline: React.FC<ElectionTimelineProps> = React.memo(({ onAskAI }) => {
  ...
  const handleKeyDown = (e: React.KeyboardEvent, index: number) => {
    if (e.key === 'ArrowRight') {
      ...
      const prevIndex = (index - 1 + timelineData.length) % timelineData.length;
      stageRefs.current[prevIndex]?.focus();
    }
  };

  return (
    <div className="w-full mb-10 relative group" role="region" aria-label="Indian election schedule">
      ...
      <div 
        ...
        tabIndex={0}
        aria-label="Scroll to see all election stages"
      ...
      >
        {timelineData.map((stage, idx) => (
          <React.Fragment key={stage.id}>
            <article aria-label={`Event: ${stage.label}`}>
              <motion.button
                ...
                className={`flex-shrink-0 flex flex-col items-center p-4 md:p-5 rounded-2xl transition-all duration-300 border-2 w-[150px] md:w-[180px] min-h-[140px] md:min-h-[160px] relative ${
              ...
              >
                {/* Status Badge */}
                <div 
                  className={`absolute -top-2 px-2 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider shadow-sm ${
                    stage.status === 'completed' ? 'bg-green-100 text-green-700' :
                    stage.status === 'ongoing' ? 'bg-blue-100 text-blue-700 animate-pulse' :
                ...
                <span className="text-2xl md:text-3xl mb-2 mt-2" role="img" aria-hidden="true">{stage.icon}</span>
                ...
                {/* Date Label */}
                <span className="text-[10px] font-bold text-gray-400 mt-2" aria-label={`Date: ${stage.date}`}>
                  {stage.date}
                </span>
              </motion.button>
            </article>
      ...
      {activeData && (
        <div 
          id={`stage-detail-${activeData.id}`}
          role="region" 
          aria-label={`Details for ${activeData.label}`}
          className="mt-4 bg-white rounded-3xl p-6 shadow-lg border border-orange-100 animate-in fade-in slide-in-from-top-4 duration-300"
        >
          <div className="flex items-start justify-between mb-4">
            <div>
              <h3 className="text-xl font-bold text-gray-900 flex items-center gap-2">
                <span role="img" aria-hidden="true">{activeData.icon}</span> {activeData.label}
              </h3>
              <div className="flex items-center gap-3 mt-1">
                <p className="text-orange-600 font-medium">{activeData.summary}</p>
                <span className="text-gray-300">|</span>
                <span className="text-sm font-bold text-gray-500 uppercase tracking-widest">{activeData.date}</span>
              </div>
            </div>
          </div>
          ...
          <button
            ...
            aria-label={`Ask VoteSaathi about ${activeData.label}`}
            className="w-full bg-gray-900 text-white py-3 rounded-xl font-semibold hover:bg-black transition-colors flex items-center justify-center gap-2 py-3 px-4"
          >
            Ask AI about this →
          </button>
        </div>
